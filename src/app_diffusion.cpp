...
//marshalltc
    if (narg < 3 || narg > 6) error->all(FLERR,"Illegal barrier command");
		if(barrier = hbarrier){
			...
			int atomtype = atoi(arg[1])-1;
			
			for(int i = 2; i < narg; i++){
				barrier[atomtype][i-2] = atof(arg[i]);
			}
...
//marshalltc
	...
	double ntotal = 0;
    //double ntype[num_types];
	double ntype[3] = {0,0,0};
	double percent_atom[num_types];
	double barrier_eff = 0;
	int site_type = iarray[1][i] - 1;

    for (int j = 0; j < numneigh[i]; j++){
		for(int k = 0; k < num_types; k++){
		  if (iarray[1][neighbor[i][j]] == k + 1) ntype[k]++;
		}
	}
	
	for(int k = 0; k < num_types; k++) ntotal += ntype[k];
	for(int k = 0; k < num_types; k++) percent_atom[k] = ntype[k]/ntotal;
	for(int k = 0; k < num_types; k++) barrier_eff += percent_atom[k]*barrier[site_type][k];

		
 
           if (edelta <= 0.0)
           probone = exp(-barrier_eff * t_inverse);
           else
