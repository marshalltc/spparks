...
for (int i = 5000; i <= 9999; i++) iarray[1][i] = 3;
...
//my edits here 
     ...
     if(iarray[1][i]==1)
        {
            if (edelta <= 0.0)
   	    probone = exp(-(barrier[ncoord(i)-delta][ncoord(j)])/1*t_inverse)/2;
            else
   	    probone = exp((-edelta-(barrier[ncoord(i)-delta][ncoord(j)]))/1*t_inverse)/2;
        }
     else
        {
           if (edelta <= 0.0)
           probone = exp(-barrier[ncoord(i)-delta][ncoord(j)]*t_inverse);
     	   ...
     	   probone = exp((-edelta-barrier[ncoord(i)-delta][ncoord(j)])*t_inverse);
...
//my code inserted here, setting particle id
iarray[1][m] = pid;

//code about coordination 

//for(int x = 0; x < 125000; x++) iarray[2][x] = ncoord(x);
...
// some more code about diffusion from me
